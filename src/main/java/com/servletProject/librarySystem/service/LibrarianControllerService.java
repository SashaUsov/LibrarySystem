...
public class LibrarianControllerService {
    ...
    public List<OnlineOrderModel> getListOfCompletedOrdersByReader(String email, String librarian) {
        var user = userService.getUserByNickName(librarian);
        if (user.getRoles().contains(Role.LIBRARIAN)) {
            var idUser = userService.getUserIdByEmail(email);
            var completedOrders = completedOrdersService.findAllByUserId(idUser);
    ...
    public List<OnlineOrderModel> getListOfCompletedOrdersByUser(String nickName) {
        ...
        var completedOrders = completedOrdersService.findAllByUserId(user.getId());
        return prepareListOfCompletedOrders(completedOrders);
    ...
    public List<OnlineOrderModel> getListOfAllCompletedOrders(String librarian) {
        var user = userService.getUserByNickName(librarian);
        if (user.getRoles().contains(Role.LIBRARIAN)) {
            var completedOrders = completedOrdersService.findAllCompletedOrders();
            return prepareListOfCompletedOrders(completedOrders);
        } else throw new PermissionToActionIsAbsentException("You do not have permission.");
    }
    ...
    public List<ArchiveBookModel> getListOfActiveUsageByUser(String email, String librarian) {
        var user = userService.getUserByNickName(librarian);
        if (user.getRoles().contains(Role.LIBRARIAN)) {
            var entity = userService.getUserByEmail(email);
            var bookUsageList = usageService.findAllByUserId(entity.getId());
            var copyIdList = getBookCopyIdFromArchiveUsageList(bookUsageList);
            var copyBookList = copiesOfBooksService.findAllById(copyIdList);
    ...
    public List<ArchiveBookModel> getListOfAllArchiveUsage(String librarian) {
        var user = userService.getUserByNickName(librarian);
        if (user.getRoles().contains(Role.LIBRARIAN)) {
            var allUsageArchive = usageService.findAllUsageArchive();
            var copyIdFromArchiveUsageList = getBookCopyIdFromArchiveUsageList(allUsageArchive);
            var copiesOfBooks = copiesOfBooksService.findAllById(copyIdFromArchiveUsageList);
            var bookIdList = OrdersUtil.getBookIdFromBookCopyList(copiesOfBooks);
            var bookCatalogList = bookCatalogService.findAllByIdIn(bookIdList);
    ...
    public void returnBookToTheCatalog(ReturnOrderInCatalogModel model, String librarian) {
        var user = userService.getUserByNickName(librarian);
        if (user.getRoles().contains(Role.LIBRARIAN)) {
            var readerId = model.getIdReader();
            var copyId = model.getIdCopy();
    ...
    private List<OnlineOrderModel> prepareListOfCompletedOrders(List<CompletedOrders> completedOrders) {
        var ordersList = getBookCopyIdFromCompletedOrdersList(completedOrders);
    ...
    private List<ArchiveBookModel> getArchiveBookModelList(List<ArchiveBookUsage> allUsageArchive,
        ...
        for (var book : bookCatalogList) {
            var model = getArchiveBookModelEntity(allUsageArchive, copiesOfBooks, book);
            modelList.add(model);
        }
        return modelList;
    }
    ...
    private ArchiveBookModel getArchiveBookModelEntity(List<ArchiveBookUsage> allUsageArchive,
        ...
        var model = new ArchiveBookModel();
        var copy = CreateEntityUtil.getCopiesOfBooksId(copiesOfBooks, book);
        model.setUniqueId(copy.getId());
        var archiveBookUsage = CreateEntityUtil.getArchiveBookUsage(allUsageArchive, copy);
